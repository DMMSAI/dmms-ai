---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Dryads AI deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Dryads AI cron scheduling for periodic checks, or version status checks on a machine running Dryads AI (laptop, workstation, Pi, VPS).
---

# Dryads AI Host Hardening

## Overview

Assess and harden the host running Dryads AI, then align it to a user-defined risk tolerance without breaking access. Use Dryads AI security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Dryads AI changes the host firewall, SSH, or OS updates; it does not.
...
5. Dryads AI gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Dryads AI can access sensitive data.
...
### 2) Run Dryads AI security audits (read-only)

As part of the default read-only checks, run `dryads-ai security audit --deep`. Only offer alternatives if the user requests them:

1. `dryads-ai security audit` (faster, non-probing)
2. `dryads-ai security audit --json` (structured output)

Offer to apply Dryads AI safe defaults (numbered):

1. `dryads-ai security audit --fix`

Be explicit that `--fix` only tightens Dryads AI defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Dryads AI version/update status (read-only)

As part of the default read-only checks, run `dryads-ai update status`.
...
- Credential hygiene notes (location of Dryads AI creds, prefer disk encryption)
...
- Dryads AI security audit (re-run)
...
After Dryads AI install or first hardening pass, run at least one baseline audit and version check:

- `dryads-ai security audit`
- `dryads-ai security audit --deep`
- `dryads-ai update status`

Ongoing monitoring is recommended. Use the Dryads AI cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `dryads-ai cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `dryads-ai update status`
...
Before creating, `dryads-ai cron list` and match on exact `name`. If found, `dryads-ai cron edit <id> ...`.
If not found, `dryads-ai cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `dryads-ai update status` (preferred for source checkouts and channels)
2. `npm view dryads-ai version` (published npm version)

## Dryads AI command accuracy

Use only supported commands and flags:

- `dryads-ai security audit [--deep] [--fix] [--json]`
- `dryads-ai status` / `dryads-ai status --deep`
- `dryads-ai health --json`
- `dryads-ai update status`
- `dryads-ai cron add|list|runs|run`

Do not invent CLI flags or imply Dryads AI enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Dryads AI compaction:
