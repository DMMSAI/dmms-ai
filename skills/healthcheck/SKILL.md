---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for DMMS AI deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, DMMS AI cron scheduling for periodic checks, or version status checks on a machine running DMMS AI (laptop, workstation, Pi, VPS).
---

# DMMS AI Host Hardening

## Overview

Assess and harden the host running DMMS AI, then align it to a user-defined risk tolerance without breaking access. Use DMMS AI security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim DMMS AI changes the host firewall, SSH, or OS updates; it does not.
...
5. DMMS AI gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if DMMS AI can access sensitive data.
...
### 2) Run DMMS AI security audits (read-only)

As part of the default read-only checks, run `dmms-ai security audit --deep`. Only offer alternatives if the user requests them:

1. `dmms-ai security audit` (faster, non-probing)
2. `dmms-ai security audit --json` (structured output)

Offer to apply DMMS AI safe defaults (numbered):

1. `dmms-ai security audit --fix`

Be explicit that `--fix` only tightens DMMS AI defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check DMMS AI version/update status (read-only)

As part of the default read-only checks, run `dmms-ai update status`.
...
- Credential hygiene notes (location of DMMS AI creds, prefer disk encryption)
...
After DMMS AI install or first hardening pass, run at least one baseline audit and version check:

- `dmms-ai security audit`
- `dmms-ai security audit --deep`
- `dmms-ai update status`

Ongoing monitoring is recommended. Use the DMMS AI cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `dmms-ai cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `dmms-ai update status`
...
Before creating, `dmms-ai cron list` and match on exact `name`. If found, `dmms-ai cron edit <id> ...`.
If not found, `dmms-ai cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `dmms-ai update status` (preferred for source checkouts and channels)
2. `npm view dmms-ai version` (published npm version)

## DMMS AI command accuracy

Use only supported commands and flags:

- `dmms-ai security audit [--deep] [--fix] [--json]`
- `dmms-ai status` / `dmms-ai status --deep`
- `dmms-ai health --json`
- `dmms-ai update status`
- `dmms-ai cron add|list|runs|run`

Do not invent CLI flags or imply DMMS AI enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by DMMS AI compaction:
