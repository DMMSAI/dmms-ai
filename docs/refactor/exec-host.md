...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.dryads-ai/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.dryads-ai/node.json` (node id + pairing token).
...
Path: `~/.dryads-ai/exec-approvals.json`
...
- Unix socket at `~/.dryads-ai/exec-approvals.sock` (0600).
