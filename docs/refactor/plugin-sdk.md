...
- Publish as `dmms-ai/plugin-sdk` (or export from core under `dmms-ai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `DmmsAiPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: DmmsAiConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: DmmsAiConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: DmmsAiConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: DmmsAiConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: DmmsAiConfig): string;
  };
};
```
...
- Introduce `dmms-ai/plugin-sdk`.
- Add `api.runtime` to `DmmsAiPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `dmmsAiRuntime: ">=2026.2.0"`).
