...
- Publish as `dryads-ai/plugin-sdk` (or export from core under `dryads-ai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `DryadsAiPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: DryadsAiConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: DryadsAiConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: DryadsAiConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: DryadsAiConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: DryadsAiConfig): string;
  };
};
```
...
- Introduce `dryads-ai/plugin-sdk`.
- Add `api.runtime` to `DryadsAiPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `dryadsAiRuntime: ">=2026.2.0"`).
