...
DMMS AI can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere DMMS AI can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, DMMS AI defaults
...
TTS config lives under `messages.tts` in `dmms-ai.json`.
...
- If `provider` is **unset**, DMMS AI prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.dmms-ai/settings/tts.json`, override with `DMMS_AI_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, DMMS AI retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, DMMS AI:
...
Discord note: `/tts` is a built-in Discord command, so DMMS AI registers
