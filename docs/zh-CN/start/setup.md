...
summary: 设置指南：在保持最新的同时保持你的 Dryads AI 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.dryads-ai/workspace`（工作区）+ `~/.dryads-ai/dryads-ai.json`（配置）。
...
- **配置：** `~/.dryads-ai/dryads-ai.json`（JSON/JSON5 格式）
- **工作区：** `~/.dryads-ai/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm dryads-ai setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **DryadsAi.app**（菜单栏）。
...
- 运行 `dryads-ai setup`，然后 `dryads-ai channels login`，然后手动启动 Gateway 网关（`dryads-ai gateway`）。
...
- **状态存储位置：**
  ...
  - 会话：`~/.dryads-ai/agents/<agentId>/sessions/`
  - 日志：`/tmp/dryads-ai/`
...
- **配对允许列表**：`~/.dryads-ai/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.dryads-ai/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.dryads-ai/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.dryads-ai/workspace` 和 `~/.dryads-ai/` 保持为"你的东西"；不要将个人提示/配置放入 `dryads-ai` 仓库。
...
- [Dryads AI 助手设置](/start/dryads-ai)
