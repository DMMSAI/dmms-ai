...
read_when:
  - 安装 DMMS AI
  - 你想从 GitHub 安装
summary: 安装 DMMS AI（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 dmms-ai、node-llama-cpp、sharp 等
pnpm add -g dmms-ai@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm dmms-ai ...` 运行仓库命令。
...
- 运行新手引导：`dmms-ai onboard --install-daemon`
- 快速检查：`dmms-ai doctor`
- 检查 Gateway 网关健康状态：`dmms-ai status` + `dmms-ai health`
- 打开仪表板：`dmms-ai dashboard`
...
```bash
# 显式 npm
curl -fsSL https://dmms-ai.com/install.sh | bash -s -- --install-method npm
...
- `DMMS_AI_GIT_DIR=...`
- `DMMS_AI_GIT_UPDATE=0|1`
- `DMMS_AI_NO_PROMPT=1`
- `DMMS_AI_DRY_RUN=1`
- `DMMS_AI_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `dmms-ai`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `dmms-ai`）。
