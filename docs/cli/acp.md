...
Run the ACP (Agent Client Protocol) bridge that talks to a Dryads AI Gateway.
...
it to drive a Dryads AI Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `dryads-ai acp` over stdio.
...
- `--server <command>`: ACP server command (default: `dryads-ai`).
