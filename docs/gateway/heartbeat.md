...
- During heartbeat runs, DMMS AI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `dmms-ai sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, DMMS AI skips the heartbeat run entirely (no model call).
