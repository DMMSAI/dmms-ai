...
read_when:
  - Running Dryads AI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Dryads AI
...
- You run Dryads AI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Dryads AI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Dryads AI extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://dryads-ai.example.com
    to: http://dryads-ai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
dryads-ai.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy dryads-ai:18789 {
...
location / {
    ...
    proxy_pass http://dryads-ai:18789;
...
`dryads-ai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Dryads AI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `dryads-ai security audit` and review findings
