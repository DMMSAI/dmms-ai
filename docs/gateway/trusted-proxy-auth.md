...
read_when:
  - Running DMMS AI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of DMMS AI
...
- You run DMMS AI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. DMMS AI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. DMMS AI extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://dmms-ai.example.com
    to: http://dmms-ai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`dmms-ai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update DMMS AI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `dmms-ai security audit` and review findings
