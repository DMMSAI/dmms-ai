...
Deploy Dryads AI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/dryads-ai/dryads-ai)
...
repository configures everything needed to run Dryads AI:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: DRYADS_AI_STATE_DIR
        value: /data/.dryads-ai
      - key: DRYADS_AI_WORKSPACE_DIR
        value: /data/workspace
      - key: DRYADS_AI_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/dryads-ai`.
...
If you use the original Dryads AI repository, Render will not auto-deploy your Dryads AI. To update it, run a manual Blueprint sync from the dashboard.
...
For Dryads AI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Dryads AI host.
