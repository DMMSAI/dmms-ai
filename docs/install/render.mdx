...
Deploy DMMS AI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/dmms-ai/dmms-ai)
...
repository configures everything needed to run DMMS AI:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: DMMS_AI_STATE_DIR
        value: /data/.dmms-ai
      - key: DMMS_AI_WORKSPACE_DIR
        value: /data/workspace
      - key: DMMS_AI_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/dmms-ai`.
...
If you use the original DMMS AI repository, Render will not auto-deploy your DMMS AI. To update it, run a manual Blueprint sync from the dashboard.
...
For DMMS AI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any DMMS AI host.
