---
summary: "Use MiniMax M2.1 in DMMS AI"
read_when:
  - You want MiniMax models in DMMS AI
...
```bash
dmms-ai plugins enable minimax-portal-auth  # skip if already loaded.
dmms-ai gateway restart  # restart if gateway is already running
dmms-ai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/dmms-ai/dmms-ai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `dmms-ai.json`:
...
- Use `dmms-ai models list` and `dmms-ai models set minimax/MiniMax-M2.1` to switch.
...
- Running `dmms-ai configure` and selecting **MiniMax M2.1**, or
