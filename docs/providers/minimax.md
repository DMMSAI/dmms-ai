---
summary: "Use MiniMax M2.1 in Dryads AI"
read_when:
  - You want MiniMax models in Dryads AI
...
```bash
dryads-ai plugins enable minimax-portal-auth  # skip if already loaded.
dryads-ai gateway restart  # restart if gateway is already running
dryads-ai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/dryads-ai/dryads-ai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `dryads-ai.json`:
...
- Use `dryads-ai models list` and `dryads-ai models set minimax/MiniMax-M2.1` to switch.
...
- Running `dryads-ai configure` and selecting **MiniMax M2.1**, or
