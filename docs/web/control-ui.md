...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/dmms-ai`)
...
If the page fails to load, start the Gateway first: `dmms-ai gateway`.
...
you revoke it with `dmms-ai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.dmms-ai/dmms-ai.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. DMMS AI
...
DMMS AI **blocks** Control UI connections without device identity.
