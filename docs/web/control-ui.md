...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/dryads-ai`)
...
If the page fails to load, start the Gateway first: `dryads-ai gateway`.
...
you revoke it with `dryads-ai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.dryads-ai/dryads-ai.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Dryads AI
...
Dryads AI **blocks** Control UI connections without device identity.
