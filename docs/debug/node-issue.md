...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in DMMS AI
...
Running DMMS AI via Node with `tsx` fails at startup with:

```
[dmms-ai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `dmms-ai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch dmms-ai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node dmms-ai.mjs status` works on Node 25.
