...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Dryads AI
...
Running Dryads AI via Node with `tsx` fails at startup with:

```
[dryads-ai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `dryads-ai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch dryads-ai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node dryads-ai.mjs status` works on Node 25.
