---
summary: "Agent tool surface for DMMS AI (browser, canvas, nodes, message, cron) replacing legacy `dmms-ai-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `dmms-ai-*` skills
title: "Tools"
---

# Tools (DMMS AI)

DMMS AI exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `dmms-ai-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `dmms-ai.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, DMMS AI logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`dmms-ai node run`).
...
DMMS AI tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `dmms-ai configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated DMMS AI-managed browser.
...
- Quick smoke: `dmms-ai nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `dmms-ai gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, DMMS AI clamps `tools.sessions.visibility` to `tree`.
