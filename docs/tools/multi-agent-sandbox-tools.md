...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `dmms-ai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `dmms-ai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=dmms-ai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${DMMS_AI_STATE_DIR:-$HOME/.dmms-ai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
