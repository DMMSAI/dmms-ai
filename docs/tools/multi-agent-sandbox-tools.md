...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `dryads-ai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `dryads-ai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=dryads-ai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${DRYADS_AI_STATE_DIR:-$HOME/.dryads-ai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
