---
title: Lobster
summary: "Typed workflow runtime for Dryads AI with resumable approval gates."
description: Typed workflow runtime for Dryads AI — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Dryads AI run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Dryads AI runs one Lobster tool call and gets a structured result.
...
Dryads AI launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | dryads-ai.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
dryads-ai.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Dryads AI tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Dryads AI Gateway (see the [Lobster repo](https://github.com/dryads-ai/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Dryads AI keeps core tools enabled. To restrict core
...
→ dryads-ai calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ dryads-ai calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Dryads AI tools that do.
