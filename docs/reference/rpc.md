...
DMMS AI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- DMMS AI owns lifecycle when `channels.signal.autoStart=true`.
...
- DMMS AI spawns `imsg rpc` as a child process (legacy iMessage integration).
