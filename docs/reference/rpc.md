...
Dryads AI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Dryads AI owns lifecycle when `channels.signal.autoStart=true`.
...
- Dryads AI spawns `imsg rpc` as a child process (legacy iMessage integration).
