...
read_when:
  - Designing workspace memory (~/.dryads-ai/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Dryads AI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.dryads-ai/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.dryads-ai/workspace` as canonical human-readable memory.
...
```
~/.dryads-ai/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Dryads AI**, but keep a separable core library.

### Why integrate into Dryads AI?

- Dryads AI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `dryads-ai memory recall "…" --k 25 --since 30d`
  - `dryads-ai memory reflect --since 7d`
...
Pragmatic take for `~/.dryads-ai/workspace`:
