...
read_when:
  - Designing workspace memory (~/.dmms-ai/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep DMMS AI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.dmms-ai/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.dmms-ai/workspace` as canonical human-readable memory.
...
```
~/.dmms-ai/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in DMMS AI**, but keep a separable core library.

### Why integrate into DMMS AI?

- DMMS AI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `dmms-ai memory recall "…" --k 25 --since 30d`
  - `dmms-ai memory reflect --since 7d`
...
Pragmatic take for `~/.dmms-ai/workspace`:
