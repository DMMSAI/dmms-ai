...
- `/usage cost` in chats: local cost summary aggregated from DMMS AI session logs.
- CLI: `dmms-ai status --usage` prints a full per-provider breakdown.
- CLI: `dmms-ai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
