...
- `/usage cost` in chats: local cost summary aggregated from Dryads AI session logs.
- CLI: `dryads-ai status --usage` prints a full per-provider breakdown.
- CLI: `dryads-ai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
