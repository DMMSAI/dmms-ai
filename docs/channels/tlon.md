...
Tlon is a decentralized messenger built on Urbit. DMMS AI connects to your Urbit ship and can
...
```bash
dmms-ai plugins install @dmms-ai/tlon
```

Local checkout (when running from a git repo):

```bash
dmms-ai plugins install ./extensions/tlon
```
...
By default, DMMS AI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `dmms-ai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, DMMS AI replies in-thread.
