...
Tlon is a decentralized messenger built on Urbit. Dryads AI connects to your Urbit ship and can
...
```bash
dryads-ai plugins install @dryads-ai/tlon
```

Local checkout (when running from a git repo):

```bash
dryads-ai plugins install ./extensions/tlon
```
...
By default, Dryads AI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `dryads-ai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Dryads AI replies in-thread.
