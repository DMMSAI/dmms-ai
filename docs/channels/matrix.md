...
Matrix is an open, decentralized messaging protocol. Dryads AI connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `dryads-ai plugins install @dryads-ai/matrix`
   - From a local checkout: `dryads-ai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Dryads AI calls the same
     login endpoint, stores the access token in `~/.dryads-ai/credentials/matrix/credentials.json`,
...
- On first connection, Dryads AI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Dryads AI logs a warning.
...
`~/.dryads-ai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Dryads AI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
