...
Matrix is an open, decentralized messaging protocol. DMMS AI connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `dmms-ai plugins install @dmms-ai/matrix`
   - From a local checkout: `dmms-ai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: DMMS AI calls the same
     login endpoint, stores the access token in `~/.dmms-ai/credentials/matrix/credentials.json`,
...
- On first connection, DMMS AI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  DMMS AI logs a warning.
...
`~/.dmms-ai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, DMMS AI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
