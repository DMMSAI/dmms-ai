...
```bash
dmms-ai plugins install @dmms-ai/msteams
```

Local checkout (when running from a git repo):

```bash
dmms-ai plugins install ./extensions/msteams
```
...
3. Configure DMMS AI with those credentials.
...
- Talk to DMMS AI via Teams DMs, group chats, or channels.
...
- On startup, DMMS AI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.dmms-ai/dmms-ai.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring DMMS AI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                   |
   | ------------------ | ------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `dmms-ai-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `dmms-ai plugins install @dmms-ai/msteams`
   - From a local checkout: `dmms-ai plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "DMMS AI in Teams", "full": "DMMS AI in Teams" },
...
DMMS AI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, DMMS AI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/DmmsAiShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

DMMS AI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `dmms-ai message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.dmms-ai/msteams-polls.json`.
...
```bash
dmms-ai message send --channel msteams \
...
dmms-ai message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
dmms-ai message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
dmms-ai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
dmms-ai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
