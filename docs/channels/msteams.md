...
```bash
dryads-ai plugins install @dryads-ai/msteams
```

Local checkout (when running from a git repo):

```bash
dryads-ai plugins install ./extensions/msteams
```
...
3. Configure Dryads AI with those credentials.
...
- Talk to Dryads AI via Teams DMs, group chats, or channels.
...
- On startup, Dryads AI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.dryads-ai/dryads-ai.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Dryads AI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                     |
   | ------------------ | --------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `dryads-ai-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `dryads-ai plugins install @dryads-ai/msteams`
   - From a local checkout: `dryads-ai plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Dryads AI in Teams", "full": "Dryads AI in Teams" },
...
Dryads AI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Dryads AI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/DryadsAiShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Dryads AI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `dryads-ai message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.dryads-ai/msteams-polls.json`.
...
```bash
dryads-ai message send --channel msteams \
...
dryads-ai message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
dryads-ai message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
dryads-ai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
dryads-ai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
