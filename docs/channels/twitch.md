...
read_when:
  - Setting up Twitch chat integration for DMMS AI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. DMMS AI connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `DMMS_AI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "dmms-ai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use DMMS_AI_TWITCH_ACCESS_TOKEN env var)
...
```bash
DMMS_AI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
