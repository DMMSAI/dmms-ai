...
read_when:
  - Setting up Twitch chat integration for Dryads AI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Dryads AI connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `DRYADS_AI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "dryads-ai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use DRYADS_AI_TWITCH_ACCESS_TOKEN env var)
...
```bash
DRYADS_AI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
