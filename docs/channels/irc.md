---
title: IRC
description: Connect Dryads AI to IRC channels and direct messages.
---

Use IRC when you want Dryads AI in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.dryads-ai/dryads-ai.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Dryads AI defaults to **mention-gating** in group contexts.
