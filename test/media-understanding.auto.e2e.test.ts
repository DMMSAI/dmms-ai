...
import type { DryadsAiConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("dryads-ai-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("dryads-ai-bin-sherpa-");
      const modelDir = await makeTempDir("dryads-ai-sherpa-model-");
      ...
      const cfg: DryadsAiConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("dryads-ai-bin-whispercpp-");
      const modelDir = await makeTempDir("dryads-ai-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("dryads-ai-bin-gemini-");
      ...
      const cfg: DryadsAiConfig = { tools: { media: { image: {} } } };
