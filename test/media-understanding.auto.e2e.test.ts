...
import type { DmmsAiConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("dmms-ai-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("dmms-ai-bin-sherpa-");
      const modelDir = await makeTempDir("dmms-ai-sherpa-model-");
      ...
      const cfg: DmmsAiConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("dmms-ai-bin-whispercpp-");
      const modelDir = await makeTempDir("dmms-ai-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("dmms-ai-bin-gemini-");
      ...
      const cfg: DmmsAiConfig = { tools: { media: { image: {} } } };
