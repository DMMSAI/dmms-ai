...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub DMMS_AI_STATE_DIR.
    // Also keep it outside os.tmpdir() so tmpdir localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["DMMS_AI_STATE_DIR"]);
    process.env.DMMS_AI_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "dmms-ai-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows default DMMS AI state workspace and sandbox roots", async () => {
  ...
  it("rejects default DMMS AI state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
