...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.DMMS_AI_BUNDLED_HOOKS_DIR;
    process.env.DMMS_AI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.DMMS_AI_BUNDLED_HOOKS_DIR;
    } else {
      process.env.DMMS_AI_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
