...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => DmmsAiConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("dmms-ai-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): DmmsAiConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
