...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => DryadsAiConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("dryads-ai-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): DryadsAiConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
