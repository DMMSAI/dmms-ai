...
export function isCommandEnabled(cfg: DryadsAiConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DryadsAiConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: DryadsAiConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DryadsAiConfig) {
