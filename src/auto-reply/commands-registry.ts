...
export function isCommandEnabled(cfg: DmmsAiConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DmmsAiConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: DmmsAiConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DmmsAiConfig) {
