...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "dryads_ai_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "dryads_ai_cmdarg", "arg-menu options");
  });
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("dryads_ai_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("dryads_ai_cmdarg_ext:");
