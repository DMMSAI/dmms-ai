...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-dmms-ai",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-dmms-ai#general");
  });
  ...
  it("derives session transcripts dir from DMMS_AI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { DMMS_AI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.DMMS_AI_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.DMMS_AI_STATE_DIR;
    const stateDir = path.resolve("/home/user/.dmms-ai");
  ...
  it("resolves cross-agent paths when DMMS_AI_STATE_DIR differs from stored paths", () => {
    const prev = process.env.DMMS_AI_STATE_DIR;
    process.env.DMMS_AI_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.DMMS_AI_STATE_DIR;
    process.env.DMMS_AI_STATE_DIR = path.resolve("/home/user/.dmms-ai");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
