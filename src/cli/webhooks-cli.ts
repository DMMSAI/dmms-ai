...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.dryads-ai.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Dryads AI hooks")
    ...
    .option("--hook-url <url>", "Dryads AI hook URL")
    .option("--hook-token <token>", "Dryads AI hook token")
