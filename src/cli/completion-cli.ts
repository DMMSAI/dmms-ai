...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Dryads AI Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(dryads-ai completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Dryads AI Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(dryads-ai completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $DRYADS_AI_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "dryads-ai") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c dryads-ai -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'dryads-ai gateway' and not 'dryads-ai other gateway', we need to check the sequence.
