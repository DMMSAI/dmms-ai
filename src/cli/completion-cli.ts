...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# DMMS AI Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(dmms-ai completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# DMMS AI Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(dmms-ai completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $DMMS_AI_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "dmms-ai") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c dmms-ai -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'dmms-ai gateway' and not 'dmms-ai other gateway', we need to check the sequence.
