    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "dmms-ai", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-dmms-ai-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-dmms-ai-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "dmms-ai", messages: [{ role: "user", content: "hi" }] },
          {
            "x-dmms-ai-agent-id": "beta",
            "x-dmms-ai-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
