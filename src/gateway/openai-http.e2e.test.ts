    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "dryads-ai", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-dryads-ai-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-dryads-ai-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "dryads-ai", messages: [{ role: "user", content: "hi" }] },
          {
            "x-dryads-ai-agent-id": "beta",
            "x-dryads-ai-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
