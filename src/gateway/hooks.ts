...
export function resolveHooksConfig(cfg: DryadsAiConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: DryadsAiConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-dryads-ai-token"] === "string"
      ? req.headers["x-dryads-ai-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
