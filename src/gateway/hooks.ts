...
export function resolveHooksConfig(cfg: DmmsAiConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: DmmsAiConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-dmms-ai-token"] === "string" ? req.headers["x-dmms-ai-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
