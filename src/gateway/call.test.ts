...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DMMS_AI_GATEWAY_TOKEN", "DMMS_AI_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    ...
    process.env.DMMS_AI_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DMMS_AI_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("prefers env password over local config password", async () => {
    process.env.DMMS_AI_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
...
describe("callGateway token resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DMMS_AI_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
