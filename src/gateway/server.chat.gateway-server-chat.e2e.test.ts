...
describe("gateway server chat", () => {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-gw-"));
