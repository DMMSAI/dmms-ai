...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.DRYADS_AI_STATE_DIR = path.join(homeDir, ".dryads-ai");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["dryads-ai", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".dryads-ai", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["DRYADS_AI_CANVAS_HOST_PORT", "DRYADS_AI_GATEWAY_TOKEN"]);
    try {
      process.env.DRYADS_AI_GATEWAY_TOKEN = "secret";
      testTailnetIPv4.value = "100.64.0.1";
      testState.gatewayBind = "lan";
      const canvasPort = await getFreePort();
      testState.canvasHostPort = canvasPort;
      process.env.DRYADS_AI_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    ...
    if (!configPath) {
      throw new Error("Missing DRYADS_AI_CONFIG_PATH");
    }
