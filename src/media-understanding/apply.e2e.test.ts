...
function createGroqAudioConfig(): DmmsAiConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): DmmsAiConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
async function createTempMediaFile(params: { fileName: string; content: Buffer | string }) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-media-"));
