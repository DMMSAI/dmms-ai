...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["DMMS_AI_BASE_ENV"]);
    process.env.DMMS_AI_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.DMMS_AI_BASE_ENV ?? "") + "|" + (process.env.DMMS_AI_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { DMMS_AI_TEST_ENV: "ok" },
        },
      );
