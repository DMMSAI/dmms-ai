...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dmms-ai-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.DMMS_AI_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "dmms-ai-telegram-"));
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as DmmsAiConfig;
    const res = resolveTelegramToken(cfg);
