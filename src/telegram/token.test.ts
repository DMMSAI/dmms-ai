...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dryads-ai-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.DRYADS_AI_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "dryads-ai-telegram-"));
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as DryadsAiConfig;
    const res = resolveTelegramToken(cfg);
