...
function createSinglePluginWithEmptySlotsConfig(): DryadsAiConfig {
  return {
    plugins: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "dryads-ai-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-dryads-ai-extensions/my-plugin",
      },
      extensionsDir,
    });
