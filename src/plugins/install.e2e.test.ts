...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `dmms-ai-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        "dmms-ai": { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      "dmms-ai": { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.dmms-ai/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without dmms-ai.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@dmms-ai/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("dmms-ai.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@dmms-ai/voice-call@0.0.1",
    });
