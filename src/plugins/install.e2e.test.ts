...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `dryads-ai-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        "dryads-ai": { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      "dryads-ai": { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.dryads-ai/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without dryads-ai.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@dryads-ai/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("dryads-ai.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@dryads-ai/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@dryads-ai/voice-call@0.0.1",
    });
