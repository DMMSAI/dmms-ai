...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/dmms-ai" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          "dmms-ai": { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchDmmsAiChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveDmmsAiUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopDmmsAiChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.DMMS_AI_GATEWAY_PORT;
    process.env.DMMS_AI_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.DMMS_AI_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.DMMS_AI_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.DMMS_AI_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.DMMS_AI_GATEWAY_TOKEN;
    } else {
      process.env.DMMS_AI_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.DMMS_AI_GATEWAY_PASSWORD;
    } else {
      process.env.DMMS_AI_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
