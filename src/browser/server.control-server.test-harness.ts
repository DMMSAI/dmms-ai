...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/dryads-ai" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          "dryads-ai": { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchDryadsAiChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveDryadsAiUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopDryadsAiChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.DRYADS_AI_GATEWAY_PORT;
    process.env.DRYADS_AI_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.DRYADS_AI_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.DRYADS_AI_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.DRYADS_AI_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.DRYADS_AI_GATEWAY_TOKEN;
    } else {
      process.env.DRYADS_AI_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.DRYADS_AI_GATEWAY_PASSWORD;
    } else {
      process.env.DRYADS_AI_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
