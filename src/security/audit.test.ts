...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: DryadsAiConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.DRYADS_AI_GATEWAY_PASSWORD;
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "dryads-ai.json");
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-dryads-ai.json");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: DryadsAiConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.DRYADS_AI_STATE_DIR;
    const tmp = await makeTmpDir("discord");
    process.env.DRYADS_AI_STATE_DIR = tmp;
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: DryadsAiConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.DRYADS_AI_GATEWAY_TOKEN;
    process.env.DRYADS_AI_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: DryadsAiConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.dryads-ai",
      configPath: "/Users/test/Dropbox/.dryads-ai/dryads-ai.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: DryadsAiConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "dryads-ai.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        "dryads-ai": { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        "dryads-ai": { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.DRYADS_AI_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.DRYADS_AI_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.DRYADS_AI_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.DRYADS_AI_GATEWAY_PASSWORD;
      } else {
        process.env.DRYADS_AI_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.DRYADS_AI_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: DryadsAiConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.DRYADS_AI_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: DryadsAiConfig = {
        gateway: {
          mode: "remote",
