...
import { POSIX_DMMS_AI_TMP_DIR, resolvePreferredDmmsAiTmpDir } from "./tmp-dmms-ai-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredDmmsAiTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `dmms-ai-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredDmmsAiTmpDir({
    accessSync,
...
describe("resolvePreferredDmmsAiTmpDir", () => {
  it("prefers /tmp/dmms-ai when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_DMMS_AI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/dmms-ai when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_DMMS_AI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/dmms-ai when /tmp/dmms-ai is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/dmms-ai when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/dmms-ai is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/dmms-ai is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/dmms-ai is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
