...
import { POSIX_DRYADS_AI_TMP_DIR, resolvePreferredDryadsAiTmpDir } from "./tmp-dryads-ai-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredDryadsAiTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `dryads-ai-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredDryadsAiTmpDir({
    accessSync,
...
describe("resolvePreferredDryadsAiTmpDir", () => {
  it("prefers /tmp/dryads-ai when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_DRYADS_AI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/dryads-ai when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_DRYADS_AI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/dryads-ai when /tmp/dryads-ai is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/dryads-ai when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/dryads-ai is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/dryads-ai is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/dryads-ai is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
