...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__dmmsai_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "dmms-ai-root");
...
describe("resolveDmmsAiPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveDmmsAiPackageRootSync } = await import("./dmms-ai-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "dmms-ai");
    const pkgRoot = path.join(project, "node_modules", "dmms-ai");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "dmms-ai" }));

    expect(resolveDmmsAiPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "dmms-ai");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "dmms-ai.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "dmms-ai" }));

    expect(resolveDmmsAiPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveDmmsAiPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-dmms-ai package roots", async () => {
    const { resolveDmmsAiPackageRootSync } = await import("./dmms-ai-root.js");

    const pkgRoot = fx("not-dmms-ai");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-dmms-ai" }));

    expect(resolveDmmsAiPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveDmmsAiPackageRoot } = await import("./dmms-ai-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "dmms-ai" }));

    await expect(resolveDmmsAiPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
