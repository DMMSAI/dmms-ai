...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("dryads-ai-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-dryads-ai-archive.tgz");
  ...
  it("rejects unsupported archive extensions", async () => {
    const dir = await createTempDir("dryads-ai-install-source-utils-");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using the final non-empty stdout line", async () => {
    const cwd = await createTempDir("dryads-ai-install-source-utils-");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: "npm notice created package\ndryads-ai-plugin-1.2.3.tgz\n",
      stderr: "",
      code: 0,
      signal: null,
      killed: false,
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "dryads-ai-plugin-1.2.3.tgz"),
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "dryads-ai-plugin@1.2.3", "--ignore-scripts"],
