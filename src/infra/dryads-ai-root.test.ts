...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__dryadsai_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "dryads-ai-root");
...
describe("resolveDryadsAiPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveDryadsAiPackageRootSync } = await import("./dryads-ai-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "dryads-ai");
    const pkgRoot = path.join(project, "node_modules", "dryads-ai");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "dryads-ai" }));

    expect(resolveDryadsAiPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "dryads-ai");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "dryads-ai.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "dryads-ai" }));

    expect(resolveDryadsAiPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveDryadsAiPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-dryads-ai package roots", async () => {
    const { resolveDryadsAiPackageRootSync } = await import("./dryads-ai-root.js");

    const pkgRoot = fx("not-dryads-ai");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-dryads-ai" }));

    expect(resolveDryadsAiPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveDryadsAiPackageRoot } = await import("./dryads-ai-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "dryads-ai" }));

    await expect(resolveDryadsAiPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
