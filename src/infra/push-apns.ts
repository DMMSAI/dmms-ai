...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.DMMS_AI_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.DMMS_AI_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set DMMS_AI_APNS_TEAM_ID and DMMS_AI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.DMMS_AI_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.DMMS_AI_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.DMMS_AI_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set DMMS_AI_APNS_PRIVATE_KEY_P8 or DMMS_AI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading DMMS_AI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
