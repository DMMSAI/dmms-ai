...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.dmmsAiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.dmmsAiCanvasA2UIAction &&
        typeof window.dmmsAiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.dmmsAiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("dmms-ai:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android DMMS AI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.dmmsAiSendUserAction === "function"
        ? window.dmmsAiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DMMS_AI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.DMMS_AI_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>DMMS AI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
