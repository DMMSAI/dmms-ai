...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.dryadsAiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.dryadsAiCanvasA2UIAction &&
        typeof window.dryadsAiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.dryadsAiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("dryads-ai:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Dryads AI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.dryadsAiSendUserAction === "function"
        ? window.dryadsAiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DRYADS_AI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.DRYADS_AI_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Dryads AI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
