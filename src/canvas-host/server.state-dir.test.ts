...
describe("canvas host state dir defaults", () => {
  ...
  it("uses DMMS_AI_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("DMMS AI Canvas");
    } finally {
      await handler.close();
