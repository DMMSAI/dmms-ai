...
describe("canvas host state dir defaults", () => {
  ...
  it("uses DRYADS_AI_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Dryads AI Canvas");
    } finally {
      await handler.close();
