...
import { createDryadsAiTools } from "./dryads-ai-tools.js";
import { __testing, createDryadsAiCodingTools } from "./pi-tools.js";
import { createDryadsAiReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createDryadsAiCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDryadsAiCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "dryads-ai-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createDryadsAiReadTool(
      baseRead as unknown as Parameters<typeof createDryadsAiReadTool>[0],
    );
