...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["DRYADS_AI_STATE_DIR", "DRYADS_AI_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-test-"));
    process.env.DRYADS_AI_STATE_DIR = stateDir;
    process.env.DRYADS_AI_PROFILE = "isolated";

    try {
      const tool = createDryadsAiTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: dryads-ai --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/dryads-ai" } }\n}\n';
