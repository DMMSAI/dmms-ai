...
const originalDryadsAiStateDir = process.env.DRYADS_AI_STATE_DIR;

afterEach(() => {
  if (originalDryadsAiStateDir === undefined) {
    delete process.env.DRYADS_AI_STATE_DIR;
  } else {
    process.env.DRYADS_AI_STATE_DIR = originalDryadsAiStateDir;
  }
});
