...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["DMMS_AI_STATE_DIR", "DMMS_AI_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-test-"));
    process.env.DMMS_AI_STATE_DIR = stateDir;
    process.env.DMMS_AI_PROFILE = "isolated";

    try {
      const tool = createDmmsAiTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: dmms-ai --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/dmms-ai" } }\n}\n';
