...
export function listAgentIds(cfg: DryadsAiConfig): string[] {
...
export function resolveDefaultAgentId(cfg: DryadsAiConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
export function resolveSessionAgentIds(params: { sessionKey?: string; config?: DryadsAiConfig }): {
...
export function resolveAgentModelPrimary(cfg: DryadsAiConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: DryadsAiConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: DryadsAiConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
