...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dryads-ai-oauth-"));

    try {
      process.env.DRYADS_AI_STATE_DIR = tempDir;
      process.env.DRYADS_AI_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.DRYADS_AI_AGENT_DIR, {
