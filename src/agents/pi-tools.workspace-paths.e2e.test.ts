...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("dmms-ai-ws-", async (workspaceDir) => {
      await withTempDir("dmms-ai-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createDmmsAiCodingTools({ workspaceDir });
    ...
    await withTempDir("dmms-ai-ws-", async (workspaceDir) => {
      await withTempDir("dmms-ai-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello dmms-ai");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("dmms-ai-ws-", async (workspaceDir) => {
      await withTempDir("dmms-ai-override-", async (overrideDir) => {
        const tools = createDmmsAiCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("dmms-ai-sandbox-", async (sandboxDir) => {
      await withTempDir("dmms-ai-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createDmmsAiCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
