...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("dryads-ai-ws-", async (workspaceDir) => {
      await withTempDir("dryads-ai-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createDryadsAiCodingTools({ workspaceDir });
    ...
    await withTempDir("dryads-ai-ws-", async (workspaceDir) => {
      await withTempDir("dryads-ai-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello dryads-ai");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("dryads-ai-ws-", async (workspaceDir) => {
      await withTempDir("dryads-ai-override-", async (overrideDir) => {
        const tools = createDryadsAiCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("dryads-ai-sandbox-", async (sandboxDir) => {
      await withTempDir("dryads-ai-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createDryadsAiCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
