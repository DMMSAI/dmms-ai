...
type WebSearchConfig = NonNullable<DmmsAiConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DmmsAiConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("dmms-ai configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.dmms-ai.com/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://dmms-ai.com",
      "X-Title": "DMMS AI Web Search",
    },
