...
type WebSearchConfig = NonNullable<DryadsAiConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DryadsAiConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("dryads-ai configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.dryads-ai.com/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://dryads-ai.com",
      "X-Title": "Dryads AI Web Search",
    },
