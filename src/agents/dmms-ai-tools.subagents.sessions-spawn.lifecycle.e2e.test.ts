...
} from "./dmms-ai-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateDmmsAiTools = (typeof import("./dmms-ai-tools.js"))["createDmmsAiTools"];
type CreateDmmsAiToolsOpts = Parameters<CreateDmmsAiTools>[0];

async function getSessionsSpawnTool(opts: CreateDmmsAiToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createDmmsAiTools } = await import("./dmms-ai-tools.js");
  const tool = createDmmsAiTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("dmms-ai-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
