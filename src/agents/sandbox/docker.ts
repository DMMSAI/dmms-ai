...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "dmms-ai.sandbox=1");
  args.push("--label", `dmms-ai.sessionKey=${params.scopeKey}`);
  args.push("--label", `dmms-ai.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `dmms-ai.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "dmms-ai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`dmms-ai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`dmms-ai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("dmms-ai sandbox recreate --all");
}
