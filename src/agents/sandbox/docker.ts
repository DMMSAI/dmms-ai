...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "dryads-ai.sandbox=1");
  args.push("--label", `dryads-ai.sessionKey=${params.scopeKey}`);
  args.push("--label", `dryads-ai.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `dryads-ai.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "dryads-ai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`dryads-ai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`dryads-ai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("dryads-ai sandbox recreate --all");
}
