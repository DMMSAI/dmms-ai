...
import { createDmmsAiTools } from "./dmms-ai-tools.js";
import { __testing, createDmmsAiCodingTools } from "./pi-tools.js";
import { createDmmsAiReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createDmmsAiCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDmmsAiCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "dmms-ai-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dmms-ai-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createDmmsAiReadTool(
      baseRead as unknown as Parameters<typeof createDmmsAiReadTool>[0],
    );
