...
} from "./dryads-ai-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateDryadsAiTools = (typeof import("./dryads-ai-tools.js"))["createDryadsAiTools"];
type CreateDryadsAiToolsOpts = Parameters<CreateDryadsAiTools>[0];

async function getSessionsSpawnTool(opts: CreateDryadsAiToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createDryadsAiTools } = await import("./dryads-ai-tools.js");
  const tool = createDryadsAiTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("dryads-ai-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
