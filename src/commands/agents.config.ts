...
export function listAgentEntries(cfg: DmmsAiConfig): AgentEntry[] {
...
function resolveAgentName(cfg: DmmsAiConfig, agentId: string) {
...
function resolveAgentModel(cfg: DmmsAiConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DmmsAiConfig): AgentSummary[] {
