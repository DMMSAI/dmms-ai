...
export function listAgentEntries(cfg: DryadsAiConfig): AgentEntry[] {
...
function resolveAgentName(cfg: DryadsAiConfig, agentId: string) {
...
function resolveAgentModel(cfg: DryadsAiConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DryadsAiConfig): AgentSummary[] {
