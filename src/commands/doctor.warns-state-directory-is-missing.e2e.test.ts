...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "dmms-ai-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.DMMS_AI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when DMMS_AI_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.DMMS_AI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
