...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveDryadsAiAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("dryads-ai models auth setup-token")}\` or \`${formatCliCommand("dryads-ai configure")}\`.`
          : `Run \`${formatCliCommand("dryads-ai configure")}\` or set an API key env var.`;
