...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveDmmsAiAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("dmms-ai models auth setup-token")}\` or \`${formatCliCommand("dmms-ai configure")}\`.`
          : `Run \`${formatCliCommand("dmms-ai configure")}\` or set an API key env var.`;
