...
function pruneAuthProfiles(
  cfg: DryadsAiConfig,
  profileIds: Set<string>,
): { next: DryadsAiConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("dryads-ai models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("dryads-ai models auth setup-token")} or ${formatCliCommand(
      "dryads-ai configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "dryads-ai models auth login --provider openai-codex",
    )} or ${formatCliCommand("dryads-ai configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("dryads-ai configure")}\` or \`${formatCliCommand("dryads-ai onboard")}\`.`;
}
