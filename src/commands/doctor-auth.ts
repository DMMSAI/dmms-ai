...
function pruneAuthProfiles(
  cfg: DmmsAiConfig,
  profileIds: Set<string>,
): { next: DmmsAiConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("dmms-ai models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("dmms-ai models auth setup-token")} or ${formatCliCommand(
      "dmms-ai configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "dmms-ai models auth login --provider openai-codex",
    )} or ${formatCliCommand("dmms-ai configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("dmms-ai configure")}\` or \`${formatCliCommand("dmms-ai onboard")}\`.`;
}
