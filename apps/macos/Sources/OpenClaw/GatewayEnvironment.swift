...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.dmmsai", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["DMMS_AI_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = DmmsAiConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.dmmsAiExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "dmms-ai CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["DMMS_AI_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "dmms-ai@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "dmms-ai@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "dmms-ai@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "dmms-ai@\(target)"])
            }

        statusHandler("Installing dmms-ai@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed dmms-ai@\(target)")
        } else {
