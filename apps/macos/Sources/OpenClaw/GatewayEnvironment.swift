...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.dryadsai", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["DRYADS_AI_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = DryadsAiConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.dryadsAiExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "dryads-ai CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["DRYADS_AI_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "dryads-ai@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "dryads-ai@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "dryads-ai@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "dryads-ai@\(target)"])
            }

        statusHandler("Installing dryads-ai@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed dryads-ai@\(target)")
        } else {
