...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.dmmsai", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous DMMS AI run already has an SSH listener on the expected port (common after restarts),
