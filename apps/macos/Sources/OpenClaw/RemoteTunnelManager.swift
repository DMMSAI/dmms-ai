...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.dryadsai", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Dryads AI run already has an SSH listener on the expected port (common after restarts),
