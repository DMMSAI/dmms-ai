...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.dryadsai", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in DryadsAiBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "ai.dryadsai.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = DryadsAiBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = DryadsAiBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = DryadsAiBonjour.wideAreaGatewayServiceDomain else { return false }
