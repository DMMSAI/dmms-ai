...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.dmmsai", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in DmmsAiBonjour.gatewayServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "ai.dmmsai.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = DmmsAiBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = DmmsAiBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = DmmsAiBonjour.wideAreaGatewayServiceDomain else { return false }
