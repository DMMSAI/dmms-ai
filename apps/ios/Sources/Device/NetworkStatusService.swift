...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> DmmsAiNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> DmmsAiNetworkStatusPayload {
        let status: DmmsAiNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> DmmsAiNetworkStatusPayload {
        DmmsAiNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
