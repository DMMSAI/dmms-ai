...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> DryadsAiNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> DryadsAiNetworkStatusPayload {
        let status: DryadsAiNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> DryadsAiNetworkStatusPayload {
        DryadsAiNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
