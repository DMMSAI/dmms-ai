...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.dmmsai.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to DMMS AI", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to DMMS AI gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to DMMS AI.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "DmmsAiShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "DMMS AI is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "DMMS AI Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("dmms-ai-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
