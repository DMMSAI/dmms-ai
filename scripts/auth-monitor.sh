...
#
# Suggested cron: */30 * * * * /home/admin/dmms-ai/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send DMMS AI notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., dmms-ai-alerts)
...
STATE_FILE="$HOME/.dmms-ai/auth-monitor-state"
...
send_notification() {
    ...
    # Send via DMMS AI if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use dmms-ai
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via DMMS AI to $NOTIFY_PHONE..."
            dmms-ai send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! DMMS AI is down. Run: ssh l36 '~/dmms-ai/scripts/mobile-reauth.sh'" "urgent"
    exit 1
