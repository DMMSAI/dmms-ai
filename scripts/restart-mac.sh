#!/usr/bin/env bash
# Reset DMMS AI like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${DMMS_AI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="DmmsAi.app/Contents/MacOS/DMMS AI"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/DMMS AI"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/DMMS AI"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/DMMS AI"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.dmmsai.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/dmms-ai-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${DMMS_AI_RESTART_LOG:-/tmp/dmms-ai-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${DMMS_AI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.dmms-ai/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  DMMS_AI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node dmms-ai.mjs daemon install --force --runtime node"
      log "  node dmms-ai.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.dmms-ai/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.dmmsai.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing DMMS AI instances"
kill_all_dmms-ai
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product DMMS AI"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/DmmsAi.app" ]]; then
    APP_BUNDLE="/Applications/DmmsAi.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/DmmsAi.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/DmmsAi.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/DmmsAi.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set DMMS_AI_APP_BUNDLE to your installed DmmsAi.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node dmms-ai.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node dmms-ai.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".dmms-ai", "dmms-ai.json"), "utf8");
