#!/usr/bin/env bash
# Reset Dryads AI like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${DRYADS_AI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="DryadsAi.app/Contents/MacOS/Dryads AI"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Dryads AI"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Dryads AI"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Dryads AI"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.dryadsai.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/dryads-ai-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${DRYADS_AI_RESTART_LOG:-/tmp/dryads-ai-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${DRYADS_AI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.dryads-ai/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  DRYADS_AI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node dryads-ai.mjs daemon install --force --runtime node"
      log "  node dryads-ai.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.dryads-ai/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.dryadsai.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Dryads AI instances"
kill_all_dryads-ai
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Dryads AI"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/DryadsAi.app" ]]; then
    APP_BUNDLE="/Applications/DryadsAi.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/DryadsAi.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/DryadsAi.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/DryadsAi.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set DRYADS_AI_APP_BUNDLE to your installed DryadsAi.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node dryads-ai.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node dryads-ai.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".dryads-ai", "dryads-ai.json"), "utf8");
