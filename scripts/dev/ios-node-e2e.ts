...
const dangerous = hasFlag("--dangerous") || process.env.DRYADS_AI_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "dryads-ai ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "dryads-ai-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "Dryads AI E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
