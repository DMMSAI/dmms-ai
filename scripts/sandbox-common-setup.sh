...
BASE_IMAGE="${BASE_IMAGE:-dmms-ai-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-dmms-ai-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=dmms-ai.sandbox=1)
NOTE
